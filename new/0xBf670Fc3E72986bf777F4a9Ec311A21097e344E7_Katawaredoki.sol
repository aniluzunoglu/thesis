...
contract Katawaredoki is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Kataware No Toki";
    string private _symbol = "KATAWAREDOKI";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x26F7d6fB4329F026b7d0D6a80359249826498F50);
    address payable private developmentWalletAddress = payable(0x26F7d6fB4329F026b7d0D6a80359249826498F50);
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    
    mapping (address => bool) public setExcludedFromFees;
    mapping (address => bool) public setWalletLimitException;
    mapping (address => bool) public setTransactionLimitExcept;
    mapping (address => bool) public setMarketPair;
    ...
    constructor () {
        ...
        setWalletLimitException[owner()] = true;
        setWalletLimitException[address(uniswapPair)] = true;
        setWalletLimitException[address(this)] = true;
        
        setTransactionLimitExcept[owner()] = true;
        setTransactionLimitExcept[address(this)] = true;

        setMarketPair[address(uniswapPair)] = true;
    ...
    function addMarketPair(address account) public onlyOwner {
        setMarketPair[account] = true;
    }

    function setsetTransactionLimitExcept(address holder, bool exempt) external onlyOwner {
        setTransactionLimitExcept[holder] = exempt;
    }
    
    function setsetExcludedFromFees(address account, bool newValue) public onlyOwner {
        setExcludedFromFees[account] = newValue;
    }
    ...
    function setsetWalletLimitException(address holder, bool exempt) external onlyOwner {
        setWalletLimitException[holder] = exempt;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {
            if(!setTransactionLimitExcept[sender] && !setTransactionLimitExcept[recipient]) {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !setMarketPair[sender] && swapAndLiquifyEnabled) 
            {
            ...
            uint256 finalAmount = (setExcludedFromFees[sender] || setExcludedFromFees[recipient]) ? 
                                         amount : takeFee(sender, recipient, amount);

            if(checkWalletLimit && !setWalletLimitException[recipient])
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(setMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
        }
        else if(setMarketPair[recipient]) {
