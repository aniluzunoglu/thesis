...
contract Staker is Ownable, ReentrancyGuard {
    ...
    uint firstRoundTime = 1 hours;
    uint secondRoundTime = 2 hours;
    uint thirdRoundTime = 3 hours;
    uint fourthRoundTime = 4 hours;
    uint fifthRoundTime = 4 hours;
    ...
    * push once to stakeholders for it to work proplerly
    ...
    function _withdrawStake(uint256 amount, uint256 index) internal returns (uint256){
        ...
        require(current_stake.since + timeDelay < block.timestamp, "Stakeable: Less than 14 days have passed since the steak was placed");
        if (current_stake.withdrawDelay + timeDelay < block.timestamp && current_stake.withdrawDelay != 0){
        ...
        require(current_stake.withdrawDelay + timeDelay < block.timestamp, "Stakeable: 14 day delay after confirmation of staking withdrawal has not passed");
