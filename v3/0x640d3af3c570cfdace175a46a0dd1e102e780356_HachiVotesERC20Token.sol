...
/**
 * @dev Intended to update the XXXX for a token based on accepting an update call from that token.
 ...
 *  Provides a function for a token to register its XXXX updater. Defaults to token itself.
 *  Provides a function a tokent to set its XXXX epoch.
 *  Implements automatic closeing and opening up a XXXX epoch when epoch ends.
 *  Provides a function to report the XXXX from the last epoch when passed a token address.
 */
interface IXXXXOracle {
  ...
  function updateXXXX( address uniV2CompatPairAddressToUpdate_, uint eopchPeriodToUpdate_ ) external returns ( bool );
}
...
contract XXXXOracleUpdater is ERC20Permit, VaultOwned {

  using EnumerableSet for EnumerableSet.AddressSet;

  event XXXXOracleChanged( address indexed previousXXXXOracle, address indexed newXXXXOracle );
  event XXXXEpochChanged( uint previousXXXXEpochPeriod, uint newXXXXEpochPeriod );
  event XXXXSourceAdded( address indexed newXXXXSource );
  event XXXXSourceRemoved( address indexed removedXXXXSource );
    
  EnumerableSet.AddressSet private _dexPoolsXXXXSources;

  IXXXXOracle public XXXXOracle;

  uint public XXXXEpochPeriod;
  ...
  function changeXXXXOracle( address newXXXXOracle_ ) external onlyOwner() {
    emit XXXXOracleChanged( address(XXXXOracle), newXXXXOracle_);
    XXXXOracle = IXXXXOracle( newXXXXOracle_ );
  }

  function changeXXXXEpochPeriod( uint newXXXXEpochPeriod_ ) external onlyOwner() {
    require( newXXXXEpochPeriod_ > 0, "XXXXOracleUpdater: XXXX Epoch period must be greater than 0." );
    emit XXXXEpochChanged( XXXXEpochPeriod, newXXXXEpochPeriod_ );
    XXXXEpochPeriod = newXXXXEpochPeriod_;
  }

  function addXXXXSource( address newXXXXSourceDexPool_ ) external onlyOwner() {
    require( _dexPoolsXXXXSources.add( newXXXXSourceDexPool_ ), "OlympusERC20TOken: XXXX Source already stored." );
    emit XXXXSourceAdded( newXXXXSourceDexPool_ );
  }

  function removeXXXXSource( address XXXXSourceToRemove_ ) external onlyOwner() {
    require( _dexPoolsXXXXSources.remove( XXXXSourceToRemove_ ), "OlympusERC20TOken: XXXX source not present." );
    emit XXXXSourceRemoved( XXXXSourceToRemove_ );
  }

  function _uodateXXXXOracle( address dexPoolToUpdateFrom_, uint XXXXEpochPeriodToUpdate_ ) internal {
    if ( _dexPoolsXXXXSources.contains( dexPoolToUpdateFrom_ )) {
      XXXXOracle.updateXXXX( dexPoolToUpdateFrom_, XXXXEpochPeriodToUpdate_ );
    }
  }

  function _beforeTokenTransfer( address from_, address to_, uint256 amount_ ) internal override virtual {
      if( _dexPoolsXXXXSources.contains( from_ ) ) {
        _uodateXXXXOracle( from_, XXXXEpochPeriod );
      } else {
        if ( _dexPoolsXXXXSources.contains( to_ ) ) {
          _uodateXXXXOracle( to_, XXXXEpochPeriod );
        }
      }
    }
}

contract Divine is XXXXOracleUpdater {
  constructor(
    string memory name_,
    string memory symbol_,
    uint8 decimals_
  ) XXXXOracleUpdater(name_, symbol_, decimals_) {
  }
}

contract HachiVotesERC20Token is Divine {

  using SafeMath for uint256;

    constructor() Divine("HachiVotes", "vCHI", 9) {
    }
