...
contract Escrow {
    ...
    function createDeal (string memory _message) external payable returns (uint) {
    ...
    function confirmPurchase (uint _dealNumber, string memory _message) external
     condition(msg.value == (2 * deals[_dealNumber].valueSeller)) payable {
    ...
    return (
        abi.encodeWithSignature("createDeal(string)"),
