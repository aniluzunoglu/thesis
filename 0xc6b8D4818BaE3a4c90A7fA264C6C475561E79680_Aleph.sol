...
contract Aleph is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
  ...
  uint256 public  PRICE = 0.08 ether; //0.08 ether presale , 0.23 eth public 

  uint256 private constant TotalCollectionSize_ = 333; // total number of nfts
  uint256 private constant MaxMintPerBatch_ = 333; //max mint per traction
  ...
  constructor() ERC721A("Aleph Panda OG-333","APOG", MaxMintPerBatch_, TotalCollectionSize_) {
    setURIbeforeReveal("ipfs://QmXNdM5q5WczC6JiHxvRJBZ7GmeCcnqCkrqxthxYuBfzf5");
  ...
  function changeMAX_Amounts(uint256 _MAX_PER_Transtion_amount, uint256 _MAX_PER_Address_amount) external onlyOwner
  {
      MAX_PER_Transtion = _MAX_PER_Transtion_amount;
      MAX_PER_Address = _MAX_PER_Address_amount;
  }
