...
contract Oldman is Ownable, ERC721A, ReentrancyGuard {
  using Strings for uint256;

  uint256 public MAX_PER_Transaction = 10; // maximam amount that user can mint/Transaction
  uint256 public TOKEN_NUMBER = 2000;

  mapping(address => uint256) public publicClaimedBy;

  uint256 private constant TotalCollectionSize_ = 8055; // total number of nfts
  ...
  constructor() ERC721A("Oldman","OLM") {
  }
  ...
  function airdrop(address sendTo, uint quantity)public onlyOwner{
    ...
    uint256 public TotalWLavailable = 8055; // total number of nfts
