...
contract TheEgyptianGods is ERC20 {
    ...
    constructor() ERC20("TheEgyptianGods", "Anunnaki") {
        owner = _msgSender();
    ...
    function setFees(uint16 _burnFee, uint16 _treasuryFee, uint8 _feeType) public onlyOwner() {
        require(_feeType < 4);
        require(_burnFee + _treasuryFee <= totalFee[3]);
        burnFee[_feeType] = _burnFee;
        treasuryFee[_feeType] = _treasuryFee;
        totalFee[_feeType] = _burnFee + _treasuryFee;
    }
    ...
    function takeFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        feeAmount = (amount * treasuryFee[feeType]) / feeDenominator;
    ...
    function getFees(uint8 _feeType) external view returns(uint16 _totalFee, uint16 _treasuryFee, uint16 _burnFee, string memory Type){
        require(_feeType < 4);
