...
contract BeetleGame is NFTokenMetadata, Ownable 
{
    // Properties
    address payable feeAddress;
    uint constant public beetlePrice = 0.2 ether;
    uint constant public maxBeetles = 4444;    // Total Beetle Supply

    // Switches
    bool public isMintingActive = true;        // Is minting open? Yes or no.
    ...
    mapping(address => uint256) public beetlesClaimed; // Map of how many Beetles are minted per address.

    // Vars
    uint256 public current_minted = 0;
    uint public mintLimit = 22;
    
    // URI Data
    string private metaAddress = "https://api.beetlegame.io/metadata/";
    ...
    // Toggle whether any beetles can be minted at all.
    ...
    // Tells the world how many Beetles a given address has minted.
    ...
    {
        require(isMintingActive, "It's not time yet.");
        require(_mintNum > 0, "Not enough beetles.");
        require(_mintNum + beetlesClaimed[msg.sender] <= mintLimit, "Cant mint more than 4 Beetles.");
        require(msg.value >= beetlePrice * _mintNum, "Need more ETH juice.");
        require(current_minted + _mintNum <= maxBeetles, "Beetles out of stock.");
        if(isWhiteListActive)
        {
            require(whiteList[msg.sender], "You're not on the lovelist.");
        }
    ...
    {
        require(_mintNum + current_minted <= maxBeetles, "Cannot mint more the total supply.");
    ...
    // Get how many Beetles are minted right now.
    function totalSupply() public view returns(uint) 
    {
        return current_minted;
    }

    // Get how many Beetles MAX can be minted.    
